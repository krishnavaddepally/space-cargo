...
Define a method on the `cargoHold` object called `fuelUpRobots`. This method should set the `fuel` value of each of our `robotsForSale` to `2`. Note that some of the robots already have some fuel in them. Any fuel given to the robots should be substracted from the fuel amount in `cargoHold.fuel`.
